        ...
        if (e.data && e.data.switch && e.data.host) {
            ...
            function openBypass(original_function) {
                return function(method, url, async) {
                    console.log(url)
                    this.requestURL = url
                    this.requestURL = this.requestURL.replace(/^http\:\/\/api-dev\.yunkezan\.com\/{1,}/, 'http://localhost:666/')
                    this.requestURL = this.requestURL.replace(/^http\:\/\/queue-dev\.yunkezan\.com\/{1,}/, 'http://localhost:666/')
